...
try:
    seen_ids_gewobag = np.load("gewobag-ids.npy").tolist()
except:
    seen_ids_gewobag = []

try:
    seen_ids_gesobau = np.load("gesobau-ids.npy").tolist()
except:
    seen_ids_gesobau = []
...
print("Gewobag seen ids: " + str(len(seen_ids_gewobag)))
print("Gesobau seen ids: " + str(len(seen_ids_gesobau)))
...
def crawl(url):
    ...
    for p in props:
        p_id = p["id"]
        if not p_id in seen_ids_gewobag:
            seen_ids_gewobag.append(p_id)
            info = p.find("a", {"class" : "angebot-header"})
            new.append(info["href"])
            print("*** found new Gewobag proposal ID: " + p_id + " ***")
            np.save("gewobag-ids.npy", seen_ids_gewobag)
    return new
...
def crawl3(url):
    ...
    for p in props:
        ...
        if not p_id in seen_ids_gesobau:
            seen_ids_gesobau.append(p_id)
            print("*** found new Gesobau proposal ID: " + p_id + " ***")
            new.append("https://www.gesobau.de" + p.find('a')['href'])
            np.save("gesobau-ids.npy", seen_ids_gesobau)
    return new
